...
def dev_funcs():
    Window.temp_frame1.update_tile('Testing New Header', 'Testing New Body')
    first_game()
    pass


def first_game():
    game = ic(get_game(sequence_number=1))
...
def get_game(**kwargs):
    ...
    if kwargs.get('name'):
        for game in steam_data:
            if game['name'] == kwargs['name']:
                return game
    elif kwargs.get('sequence_number'):
        return steam_data[kwargs['sequence_number'] - 1]
    else:
        return None
...
class DashBoard:
    ...
    def __init__(self):
        ...
        self.temp_frame6 = None
        self.temp_frame7 = None
        self.temp_frame8 = None
    ...
    def call_tiles(self):
        ...
        self.temp_frame5 = SmallTile(frame_root=self.root, frame_color='#C700FF',
                                     header="Tile5",
                                     body=f"{'test5 ' * 500}")
        # self.temp_frame6 = SmallTile(frame_root=self.root, frame_color='#C700FF',
        #                              header="Tile6",
        #                              body=f"{'test6 ' * 500}")
        # self.temp_frame7 = SmallTile(frame_root=self.root, frame_color='#C700FF',
        #                              header="Tile7",
        #                              body=f"{'test7 ' * 500}")
        # self.temp_frame8 = SmallTile(frame_root=self.root, frame_color='#C700FF',
        #                              header="Tile8",
        #                              body=f"{'test8 ' * 500}")

    def order_frames(self, grid_configuration):
        columns, rows = [int(value) for value in grid_configuration.split('x')]  # if order_frames("3x2"), columns = 3, rows = 2
        ic(columns, rows)
        for row in range(rows):
            self.root.rowconfigure(row, weight=100 // rows, uniform='root_row')
            for column in range(columns):
                self.root.columnconfigure(column, weight=100 // columns, uniform='root_column')

        current_row, correction = 0, 0
        grid_amount = columns * rows
        for index, instance in enumerate(SmallTile.instances.values()):
            if index < grid_amount:
                if index - correction >= columns:
                    current_row += 1
                    correction = columns * current_row
                instance.frame.grid(column=(index - correction), row=current_row, sticky='news', padx=20, pady=20)
            else:
                break

        self.root.update()
        ic((self.temp_frame1.__dict__['frame']).winfo_width())
        for instance in SmallTile.instances.values():
            instance.__dict__['frame'].__dict__['children']['!frame2'].__dict__['children']['!label'].config(wraplength=(instance.__dict__['frame'].winfo_width()))
            pass
...
class SmallTile:
    ...
    def __init__(self, frame_root, frame_color, header, body):
        ...
        self.render_frame(frame_color)

    def render_frame(self, frame_color):
